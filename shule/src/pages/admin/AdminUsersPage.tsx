...
export function AdminUsersPage() {
  ...
  const { data: allStaff = [], isLoading } = useStaff(useMemo(() => ({}), []))
  const { data: depts    = [] }            = useDepartments()
  const { data: school }                   = useSchoolSettings()
  const deptMap  = useMemo(() => new Map(depts.map(d => [d.id, d.name])), [depts])

  const pending = useMemo(() => allStaff.filter(s => !s.authUserId), [allStaff])
  const active  = useMemo(() => allStaff.filter(s =>  s.authUserId), [allStaff])

  function filterList(list: Staff[]) {
    let out = list
    if (roleFilter) out = out.filter(s => s.role === roleFilter)
    if (search.trim()) {
      const q = search.toLowerCase()
      out = out.filter(s => `${s.firstName} ${s.lastName}`.toLowerCase().includes(q) || s.staffNumber.toLowerCase().includes(q) || (s.email ?? '').toLowerCase().includes(q))
    }
    return out
  }

  const displayList = filterList(tab === 'pending' ? pending : active)
  const schoolName  = school?.schoolName ?? 'School'

  function onActivated(cred: CredInfo) { setNewCred(cred); setTab('active') }
    ...
    <>
      <div className="sui-page-enter" style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ borderRadius: 18, overflow: 'hidden', background: 'linear-gradient(135deg,#8b5cf6 0%,#6366f1 100%)', padding: '28px 28px 24px', position: 'relative' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              ...
              <h1 style={{ fontFamily: 'var(--font2)', fontWeight: 900, fontSize: 22, color: '#fff', margin: 0, letterSpacing: -.4 }}>Staff Logins</h1>
            </div>
            <p style={{ color: 'rgba(255,255,255,.7)', fontSize: 12.5, margin: '0 0 20px' }}>Activate system access for registered staff members</p>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {[
                { label: 'Total Staff', value: isLoading ? '—' : allStaff.length, accent: 'rgba(255,255,255,.2)' },
                { label: 'Pending', value: isLoading ? '—' : pending.length, accent: pending.length > 0 ? 'rgba(244,63,94,.45)' : 'rgba(255,255,255,.15)' },
                { label: 'Active Logins', value: isLoading ? '—' : active.length, accent: 'rgba(255,255,255,.15)' },
              ].map(s => (
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          {([
            { id: 'pending' as const, label: 'Pending Activation', count: pending.length, red: pending.length > 0 },
            { id: 'active'  as const, label: 'Active Logins',      count: active.length,  red: false },
          ]).map(t => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '0 12px', height: 38 }}>
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name, staff number or email…" style={{ border: 'none', background: 'transparent', fontSize: 12.5, color: 'var(--txt)', outline: 'none', flex: 1 }} />
            {search && <button onClick={() => setSearch('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--txt3)', display: 'flex', padding: 2 }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>}
          </div>
          <div style={{ display: 'flex', gap: 7, overflowX: 'auto', paddingBottom: 2, scrollbarWidth: 'none' }}>
            {ROLE_TABS.map(rt => {
              const active2 = roleFilter === rt.value
              const [c1] = rt.value ? roleGrad(rt.value) : ['#8b5cf6', '#6366f1']
              return (
                <button key={rt.value} onClick={() => setRoleFilter(rt.value)} style={{ flexShrink: 0, padding: '6px 14px', borderRadius: 99, border: `1px solid ${active2 ? `${c1}40` : 'var(--border)'}`, background: active2 ? `${c1}14` : 'var(--surface)', color: active2 ? c1 : 'var(--txt3)', fontWeight: active2 ? 800 : 600, fontSize: 12.5, fontFamily: 'var(--font2)', cursor: 'pointer', transition: 'all 0.14s' } as React.CSSProperties}>
                  {rt.label}
                </button>
              )
            })}
        ...
        {isLoading ? (
          ...
          <div className="stagger-cards" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 14 }}>
            {tab === 'pending'
              ? displayList.map(s => <PendingCard key={s.id} staff={s} deptName={s.departmentId ? (deptMap.get(s.departmentId) ?? null) : null} onActivated={onActivated} />)
              : displayList.map(s => <ActiveCard  key={s.id} staff={s} deptName={s.departmentId ? (deptMap.get(s.departmentId) ?? null) : null} onReset={setNewCred} onLink={(id, name) => setLinkModal({ staffId: id, staffName: name })} />)
            }
          </div>
        )}
        ...
        {!isLoading && displayList.length > 0 && (
          <div style={{ fontSize: 12, color: 'var(--txt3)', textAlign: 'center', paddingBottom: 8 }}>
            {displayList.length} {tab === 'pending' ? 'pending' : 'active'} staff{displayList.length !== 1 ? ' members' : ' member'}{(search || roleFilter) ? ' (filtered)' : ''}
          </div>
        )}
      </div>
