...
serve(async (req) => {
  ...
  try {
    // Only allow calls from authenticated staff with secretary/principal/it_admin role
    ...
    const { email, staffId, schoolId } = await req.json()
    ...
    // Verify the caller is secretary/principal/it_admin
