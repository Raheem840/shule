  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {open && matches.length > 0 && (
        <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, right: 0, zIndex: 500, background: 'var(--surface)', border: '.5px solid var(--border)', borderRadius: 14, boxShadow: '0 8px 32px rgba(0,0,0,.14)', overflow: 'hidden' }}>
          {matches.map(s => (
            <div key={s.id} onMouseDown={() => select(s)} style={{ padding: '10px 14px', cursor: 'pointer', display: 'flex', gap: 10, alignItems: 'center', borderBottom: '.5px solid var(--border)' }} className="sui-tr">
...
// ─── Record modal (centered — no bottom-sheet) ─────────────────────────────────
...
// ─── Delete modal (centered — no bottom-sheet) ─────────────────────────────────
